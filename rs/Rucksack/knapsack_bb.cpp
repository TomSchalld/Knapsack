 ...
 * argv[3] iterations for every instance 
 * argv[4] print debug
...
void recurse(int index, int costs[], int weights[], int bin_rep[]) {
    int ub = upperbound(index, costs, bin_rep);
        bin_rep[index]=1;
    if(best<ub) {
    //case1: include item at index
        if(debug==1) std::cout<<"adding "<<index<<", best "<<best<<"\n";
        ...
        bin_rep[index]=0;
    if(best<ub) {
    //case2: NOT include item at index 
        if(debug==1) std::cout<<"removing "<<index<<", best "<<best<<"\n";
        int val = eval(costs, weights, bin_rep);
        if(index+1<n){
            recurse(index+1, costs, weights, bin_rep);
        } else {
        ...
        if(debug==1) std::cout<<"regression at level "<<index<<"\n";
    }
}
